...
OPUS_D fp32x2_t pk_mul_f32(fp32x2_t a, fp32x2_t b)
{
    ...
    // CDNA2+ archs have `v_pk_mul_f32`; keep the asm form so the packed
    // instruction is guaranteed (compiler auto-vectorization is best-effort).
    fp32x2_t c;
    asm volatile("v_pk_mul_f32 %0, %1, %2" : "=v"(c) : "v"(a), "v"(b));
    return c;
#else
    // gfx908, RDNA archs (gfx10xx and later), and host have no
    // `v_pk_mul_f32`, so use the portable element-wise form.
