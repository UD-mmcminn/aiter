...
<sup>2</sup> gfx908 build targeting is enabled. MI100 hardware validation currently covers source-backed activation, CK BF16 LayerNorm, fused-add LayerNorm, and batched GEMM, fused RMSNorm plus INT8 quantization, basic and grouped top-k routing, MoE sorting through the Opus, CK, and FlyDSL backends, KV-cache reshape/quantization, native BF16 paged-attention decode (standard, v1, and ragged), Triton BF16 GEMM, and dense/packed-varlen Triton BF16 FlashAttention forward/backward. AITER ASM code objects have not been ported, and broad correctness and performance coverage is still incomplete; this is not production support yet.
