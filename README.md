...
<sup>2</sup> gfx908 build targeting is enabled. MI100 hardware validation currently covers source-backed activation, BF16 causal-conv1d update and prefill split-QKV, BF16 split-GDR decode update, and native BF16 chunked-GDR prefill state propagation, CK BF16 LayerNorm, fused-add LayerNorm, and batched GEMM, CK INT8 A8W8 GEMM with BF16/FP16 output (including odd output widths and optional bias), native BF16 GroupNorm, fused RMSNorm plus INT8 quantization, native FP16/BF16/FP32 plain top-k selection, native FP32 per-row top-k prefill/decode (including stable ordering and value output), basic and grouped top-k routing, unified sigmoid/sqrt-softplus/softmax MoE gating, MoE sorting through the Opus, CK, and FlyDSL backends, unquantized CK BF16 fused MoE, native BF16 greedy/random/mixed sampling with internal and external exponential noise, unquantized KV-cache reshape (standard, ASM-compatible, and flash layouts), block copy, and block swap, native BF16 RoPE (cached and uncached SBHD, cached positions and offsets, packed THD, and cached 2D, including backward paths where available), native BF16 paged-attention decode (standard, v1, and ragged), Triton BF16 GEMM, direct Triton BF16 MLA decode with ordinary pages, and dense/packed-varlen Triton BF16 FlashAttention forward/backward. The MLA probe exercises the Triton API directly; the public `aiter.mla.mla_decode_fwd` API is not routed through it on gfx908. AITER ASM code objects have not been ported, and broad correctness and performance coverage is still incomplete; this is not production support yet.
